...
Create a new blog post for kelvinmai.io.

## Usage

```
/blog-post [topic, description, or path to draft file]
```
...
1. **Explicit file path**: If the argument is a path ending in `.md` or `.mdx`, read that file as the draft content.
2. **Default draft**: If `content/draft.md` exists, read it and use its contents as the blog brief.
3. **Inline args only**: If neither applies, use the provided arguments directly as the topic/description.

When a draft file is found, use its full contents as the source of truth for what to write. Any additional inline arguments serve as context or overrides (e.g. a backdate or extra instruction).
